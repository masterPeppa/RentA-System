...
if(isset($_SESSION['useradmin'])){
    $selectapplication = "SELECT * FROM application_data";
    ...
    if(isset($_GET['notifid']) && $_GET['notifid'] != ""){
        ...
        $update_notif="UPDATE admin_notification SET notif_status='read' WHERE notif_info='Application'";
        $newnotif_update_executed=mysqli_query($con,$update_notif);
    }
?>
<!DOCTYPE html>
    ...
    <section class="home-section ">
        ...
        <div class="admin-container p-3">
            <div class="d-flex justify-content-center ">
                <h1 class="admin-page-h1">Applications</h1>
            </div>
                ...
                <div class="mt-5">
                    ...
                    <table role="table" class="table-admin mt-3">
                        ...
                        <thead role="rowgroup" class="headers">
                            <tr role="row" class="headers">
                                <th role="columnheader" class="py-3 pe-5 ps-xl-3">Application ID</th>
                        ...
                        <tbody role="rowgroup">
                            ...
                            for($i = 0; $i<count($row_application); $i++){
                                $selectrenter = "SELECT * FROM user_renter WHERE rId='".$row_application[$i]['renter_id']."'";
                                $executerenter = mysqli_query($con, $selectrenter);
                                $row_renter = mysqli_fetch_assoc($executerenter);

                                $selectlandlord = "SELECT * FROM user_landlord WHERE lID='".$row_application[$i]['landlord_id']."'";
                                $executelandlord = mysqli_query($con, $selectlandlord);
                                $row_landlord = mysqli_fetch_assoc($executelandlord);

                                $selectproperty = "SELECT * FROM landing_properties WHERE propertyID='".$row_application[$i]['property_id']."'";
                                $executeproperty = mysqli_query($con, $selectproperty);
                                $row_property = mysqli_fetch_assoc($executeproperty);

                                $selectlease = "SELECT * FROM lease WHERE property_id='".$row_application[$i]['property_id']."'";
                                $executelease = mysqli_query($con, $selectlease);
                                $row_lease = mysqli_fetch_assoc($executelease);
                                ?>
                                <tr role="row" class="admin-table-row" >
                                    <td role="cell" class="py-3 pe-2 ps-xl-3 admin-apps"> <b><?php echo $row_application[$i]['application_id']?></b>  </td>
                                    <td role="cell" class="py-3 pe-2 admin-apps"> 
                                        <p><?php echo $row_property['propertyType'] ?>  for Rent</p>
                                        <p><?php echo $row_property['propertyBarangay'].", ".$row_property['propertyCity'] ?></p>
                                    </td>
                                    <td role="cell" class="py-3 pe-2 admin-apps"> <?php echo $row_landlord['lLname'].", ".$row_landlord['lFname'] ?></td>
                                    <td role="cell" class="py-3 pe-3 admin-apps"> <?php echo $row_renter['rLname'].", ".$row_renter['rFname'] ?> </td>
    
                                    <td role="cell" class="py-3 pe-3 admin-apps"> 
                                        <?php
                                        if($row_application[$i]['send_status'] == "1"){
                                            ?>
                                        ...
                                        else if($row_application[$i]['send_status'] == "2"){
                                            ?>
                                        ...
                                        else if($row_application[$i]['send_status'] == "4" && $row_lease['move_in_data'] != NULL){
                                            ?>
                                            <div class="status-moving-in">
                                                <span class='status-gray'>Moving-in</span>
                                            </div>
                                            <?php
                                        }
                                        else if($row_application[$i]['send_status'] == "3"){
                                            ?>
                                            <div class="status-settle">
                                        ...
                                        else if($row_application[$i]['send_status'] == "5" && $row_lease['lease_status'] == "residing"){
                                            ?>
                                            <div class="status-residing">
                    ...
                    <div class="d-flex justify-content-center mt-4">
                        ...
                        if(count($row_application) == 0){
                            ?>
                            <p class="no-data yet">-- No application yet. --</p>
                            <?php
                        }
                        ?>
                    </div>
