...
if(isset($_SESSION['rEmail'])){
    ...
    $select_lease = "SELECT * FROM lease WHERE renter_id ='".$getUser['rId']."' AND move_in_data != '0000-00-00'";
    ...
    $getId = "SELECT * FROM application_data WHERE renter_id='".$getUser['rId']."' AND send_status!='rejected'";
    ...
    if(isset($_GET['idnotif']) && $_GET['idnotif'] != ""){
        ...
        $update_notif="UPDATE renter_notification SET notif_status='read' WHERE renter_id = '".$getUser['rId']."' AND notif_info='moved-in-q'";
        $newnotif_update_executed=mysqli_query($con,$update_notif);

        $update_notif1="UPDATE renter_notification SET notif_status='read' WHERE renter_id = '".$getUser['rId']."' AND notif_info='receipt-accepted'";
...
<!-- MAIN -->
        ...
        <section class="section-progress d-flex align-items-center justify-content-center">
            ...
            <div class="stepper-wrapper d-flex justify-content-between w-100">
                ...
                <div class="stepper-item d-flex flex-column align-items-center active">
                    ...
                    <div class="step-name mt-2 active">Move-in</div>
                </div>
    ...
    <!-- PROPERTY INFO SECTION -->
        <section class="section-empty mt-5">
            <div class="d-flex flex-column gap-5 justify-content-center align-items-center" id="divdatebody">
                
            </div>
            <div class="d-none">
                <input type="text" id="renterId" value="<?php echo $getUser['rId']?>">
                <input type="text" id="propertyId" value="<?php echo $rcheckidexistence['property_id']?>">
                <input type="text" id="idlandlord" value="<?php echo $rcheckidexistence['landlord_id']?>">
            </div>
        </section>
...
<!-- ```````````````````````````````` -->
    ...
    <script>
        ...
        setInterval(function(){
            ...
            setInterval(function () {
                $.ajax({
                    url: "../Functions/Renters/realtimechecksetdate.php",
                    method: "POST",
                    data: {
                        userid: $("#txtUserId").val(),
                        landlordid: $("#idlandlord").val()
                    },
                    dataType: "text",
                    success: function (data) {
                        $("#divdatebody").html(data);
                    }
                });
            }, 700);
...
<?php
        ...
        if(isset($_GET['date']) && $_GET['date'] != ""){
            echo "<script>window.location.href = 'starterPage.php?date=".$_GET['date']."'</script>";
        }
        else{
