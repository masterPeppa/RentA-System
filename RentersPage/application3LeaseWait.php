...
if(isset($_SESSION['rEmail'])){
    ...
    $countlease = mysqli_num_rows($execute_lease);
    ...
    if(isset($_GET['idnotif']) && $_GET['idnotif'] != ""){
        ...
        $update_notif="UPDATE renter_notification SET notif_status='read' WHERE renter_id = '".$getUser['rId']."' AND notif_info='application-approved'";
        $newnotif_update_executed=mysqli_query($con,$update_notif);
    }
    if($countlease == 0){
    ?>
    ...
    <!--  -->
        ...
        <section class="section-empty mt-5">
            <div class="d-flex flex-column gap-5 justify-content-center align-items-center">
                <h3 class="mt-5 txt-havent text-center">Sign your lease</h3>
                <p class="text-center">Please wait while the landlord is currently procesing your lease agreement. You can message him/her if you have any concerns. <br> We will notify you when your lease agreement is ready.</p>
                <a href="../rentals.php" role="button" class="btn-explore px-4 py-3 d-flex align-items-center justify-content-center gap-2 text-decoration-none">
...
<!-- ```````````````````````````````` -->
    ...
    <script>
        ...
        setInterval(function(){
                $.ajax({
                    url: "../Functions/Renters/realtimeredirect.php",
                    method: "POST",
                    dataType: "text",
                    success: function(data) {
                    if(data == "lease1"){
                        window.location.href = 'application3SignLeaseUploaded.php';
                    }
                    else if(data == "lease2"){
                        window.location.href = 'application3SignLeaseCreated.php';
                    }
...
<?php
        ...
        if($checklease['sent_status'] == "sent1"){
            echo "<script>window.location.href = 'application3SignLeaseUploaded.php'</script>";
        }
        else if($checklease['sent_status'] == "sent2"){
            echo "<script>window.location.href = 'application3SignLeaseCreated.php'</script>";
        }
    }
    }
    else{
