...
<head>
    ...
    <script>
        ...
        function gotIt(){
            ...
            var verificationinfo = document.getElementById("txtverificationinfo");
            if(landlordaction.value != "null" && verificationinfo.value == "fully-verified"){
			...
			<!-- for save condition -->
                ...
                <div class="dropdown ms-auto d-sm-block d-md-none">
                    <button onclick="dropdownAvatarFunction()" onblur="blurFunction()" class="btn btn-light dropdown-toggle d-inline-block" type="button" id="dropdrownbtn-avatar" data-bs-toggle="dropdown" aria-expanded="false">
                        <img src="<?php echo $getUser['lImgProfile'] ?>" alt="" class="img-avatar me-1">
    ...
    <?php
            if(isset($_GET['action']) && $_GET['action'] != ''){ ?>
                <div class="d-none">
                    ...
                    <input type="text" id="txtverificationinfo" value="<?php echo $$getUser['lStatus'] ?>">
                </div>
            <?php
            }
            else{
...
<?php
            $updateNewUser = "UPDATE user_landlord SET new_user='1' WHERE lID='".$getUser['lID']."' AND new_user=0";
            $executeNewuser = mysqli_query($con, $updateNewUser);
        }
        else{
