...
<?php
    ...
    if(isset($_GET['id']) && $_GET['id'] != ""){
        ...
        $update_notif="UPDATE landlord_notification SET notif_status='read' WHERE landlord_id = '".$lgetId['lID']."' AND notif_info='payment'";
        $newnotif_update_executed=mysqli_query($con,$update_notif);
    }
?>
<!DOCTYPE html>
                    ...
                    <ul class="navbar-nav navbar-nav-landlord d-flex align-items-center">
                        ...
                        <li class="nav-item dropdown d-none d-sm-none d-md-block">
                            ...
                            <ul class="dropdown-menu dropdown-menu-manage" aria-labelledby="dropdrownbtn-manage">
                                ...
                                <li><a class="dropdown-item active-dropdown" href="manageLeases.php">Leases</a></li>
                       ...
                       <li class="nav-item d-block d-sm-block d-md-none">
                        ...
                        <li class="nav-item d-block d-sm-block d-md-none">
                            <a class="nav-link active-dropdown" href="manageLeases.php">Leases</a>
                        </li>
...
<!-- MAIN -->
<?php
    $selectApplication = "SELECT * FROM application_data WHERE landlord_id='".$lgetId['lID']."' AND send_status >=2";
...
<!-- NO APPLICANTS -->
    <div class="container-fluid container-no-approved-applicants px-3 px-md-5 py-3">
        <header>
            <h3 class="title">Leases</h3>
            <p class="mt-3">You currently have no approved applicants.</p>
        </header>
...
<!-- THERE'S APPROVED APPLICANTS -->
    ...
    <?php
        ...
        <header>
            <h3 class="title">Leases</h3>
            <p class="mt-3">You currently have <span class="total-applicants"><?php echo count($row_application) ?></span> approved applicant/s.</p>
        </header>
        ...
        <!-- HEADER -->
            <thead role="rowgroup" class="headers">
                <tr role="row" class="headers">
                    ...
                    <th role="columnheader" class="py-3 pe-5">Date sent</th>
                    <th role="columnheader" class="py-3 pe-5">Date signed</th>
        ...
        <!-- DATA -->
            ...
            <?php
                ...
                <tr role="row" class="border-bottom">
                    <td role="cell" class="py-3 pe-2 manage-lease-header" id="statusBody<?php echo $i ?>">
                    <?php
                    if(count($getlistlease) == 0){
                        echo "<span class='status-signing'><i class='bi bi-circle-fill pe-1'></i></span><span class='status-signing'>No lease yet</span>";
                        $datesent = '--/--/----';
                        $datesigned = '--/--/----';
                    }
                    else{
                        $selectlease = "SELECT * FROM lease WHERE landlord_id='".$lgetId['lID']."' AND
                        renter_id='$renterId' AND property_id='".$row_application[$i]['property_id']."'";
                        $executelease = mysqli_query($con, $selectlease);
                        $getlease = mysqli_fetch_assoc($executelease);
                        $getleasecount = mysqli_num_rows($executelease);
                        if($getleasecount > 0){
                            ?>
                            <div class="d-none">
                                <input type="text" id="currentValue<?php echo $i ?>" value="<?php echo $getlease['lease_status']?>">
                            </div>
                            <?php
                            if($getlease['date_lease_sent'] == NULL){
                                $datesent = '--/--/----';
                            }
                            else{
                                $datesent = date("m-d-Y", strtotime($getlease['date_lease_sent']));
                            }
                            if($getlease['date_lease_signed'] == NULL){
                                $datesigned = '--/--/----';
                            }
                            else{
                                $datesigned = date("m-d-Y", strtotime($getlease['date_lease_signed']));
                            }
                            if($getlease['lease_status'] == "for-signing"){
                                echo "<span class='status-signing'><i class='bi bi-circle-fill pe-1'></i></span><span class='status-signing'>For Signing</span>";
                            }
                            else if($getlease['lease_status'] == "signed" || $getlease['lease_status'] == "moving-in" || $getlease['lease_status'] == "residing"){
                                echo "<span class='status-signed'><i class='bi bi-circle-fill pe-1'></i></span><span class='status-signed'>Signed</span>";
                            }
                            else if($getlease['lease_status'] == "cancelled"){
                                echo "<span class='status-cancelled'><i class='bi bi-circle-fill pe-1'></i></span><span class='status-cancelled'>Cancelled</span>";
                            }
                            else if($getlease['lease_status'] == "moved-out"){
                            ...
                            ?>
                            <div class="d-none">
                                <input type="text" id="currentValue<?php echo $i ?>" value="null">
                            </div>
                            <?php
                        }
                    }
                    ...
                    <td role="cell" class="py-3 pe-2 manage-lease-header"><?php echo $getProperty['propertyTitle'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-lease-header"><?php echo $getrenterinfo['rFname'] , " ", $getrenterinfo['rLname'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-lease-header"><?php
                        echo $datesent ?>
                    </td>
                    <td role="cell" class="py-3 pe-2 manage-lease-header"><?php 

                        echo $datesigned ?></td>
                    <td role="cell" class="py-3 manage-lease-header">
                        <div class="d-flex flex-column gap-2">
                            <div class="">
                                <!-- default na lalabas, bali naapprove na sia ni landlord -->
                                <?php
                                if($row_application[$i]['send_status'] == "2"){
                                    ?>
                                    <div class="div-send-lease">
                                        <a href="uploadLease.php?renterId=<?php echo $getrenterinfo['rId'] ?>&propertyid=<?php echo $propertyId ?>" class="btn btns-manage-applicants px-1 py-2 d-flex align-items-center justify-content-center" role="button">
                                            <i class="bi bi-send-plus-fill actionIcon pe-1"></i>
                                            <span>Send a lease</span>
                                        </a>
                                    </div>
                                    <?php
                                }
                                    ...
                                    ?> 
                                    <!-- pag nakapagsend na, etong button na ung lalabas, mapupunta to sa (uploadedAgreements.php) -->
                                    <div class="div-view-lease d-flex flex-column gap-2">
                                        <!-- <a href="send1LeaseAgreement.php?renterId=<?php echo $getrenterinfo['rId'] ?>&propertyid=<?php echo $propertyId ?>" class="d-none btn btns-manage-applicants px-1 py-2 d-flex align-items-center justify-content-center" role="button"> -->
                                        <a href="uploadedAgreements.php?renter=<?php echo $renterId ?>&property=<?php echo $propertyId?>" class="btn btns-manage-applicants px-1 py-2 d-flex align-items-center justify-content-center" role="button">    
                                            <i class="bi bi-search-heart actionIcon pe-1"></i>
                                            <span>View lease</span>
                                        </a>
                                        <a href="approveApplication.php?property=<?php echo $propertyId ?>&id=<?php echo $getrenterinfo['rId']?>&viewInfo=View" class="btn btns-manage-applicants px-1 py-2 d-flex align-items-center justify-content-center" role="button">    
                                            <i class="bi bi-search-heart actionIcon pe-1"></i>
                                            <span>Renter form</span>
                                        </a>
                                    </div>
                                    <?php
                                }
          ...
          <?php
            if(count($getlistlease) == 0){
                ?>
                    <input type="text" id="leaseValue" value="null">
                <?php
            }
            else{
                ?>
                <input type="text" id="leaseValue" value="notnull">
                <?php
            }
            ?>
            </div>
...
<?php
    ...
    <script>
            ...
            intervalId = setInterval(function () {
                if (document.getElementById('leaseValue').value != "null") {
                    $.ajax({
