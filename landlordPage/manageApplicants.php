<?php
    ...
    if(isset($_GET['id']) && $_GET['id'] != ""){
        ...
        $update_notif="UPDATE landlord_notification SET notif_status='read' WHERE landlord_id = '".$lgetId['lID']."' AND notif_info='application'";
        $newnotif_update_executed=mysqli_query($con,$update_notif);
    }
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>RentA | Manage Applicants</title>
                    ...
                    <ul class="navbar-nav navbar-nav-landlord d-flex align-items-center">
                        ...
                        <li class="nav-item dropdown d-none d-sm-none d-md-block">
                            ...
                            <ul class="dropdown-menu dropdown-menu-manage" aria-labelledby="dropdrownbtn-manage">
                                <li><a class="dropdown-item dropdown-item-first active-dropdown" href="manageApplicants.php">Applicants</a></li>
                       ...
                       <li class="nav-item d-block d-sm-block d-md-none">
                            <a class="nav-link active-dropdown" href="manageApplicants.php">Applicants</a>
                        </li>
...
<!-- MAIN -->
<?php
    $selectApplication = "SELECT * FROM application_data WHERE landlord_id='".$lgetId['lID']."' AND send_status='1'";
    $executeApplication = mysqli_query($con, $selectApplication);
    $row_application = mysqli_fetch_all($executeApplication, MYSQLI_ASSOC);

    $selectApplicationrejected = "SELECT * FROM application_data WHERE landlord_id='".$lgetId['lID']."' AND send_status='rejected'";
    $executeApplicationrejected = mysqli_query($con, $selectApplicationrejected);
    $row_applicationrejected = mysqli_fetch_all($executeApplicationrejected, MYSQLI_ASSOC);

    if(count($row_application) == 0 && count($row_applicationrejected) == 0){
    ?>
<!-- NO APPLICANTS -->
    <div class="container-fluid container-no-applicants px-3 px-md-5 py-3">
        <header>
            <h3 class="title">Manage Applicants</h3>
            <p class="mt-3">You currently have no applicants yet.</p>
        </header>
    ...
    ?>
<!-- THERE'S APPLICANTS -->
    <div class="container-fluid container-manage-applicants px-3 px-md-5 py-3">
        <?php
        if(count($row_application) > 0){
        ?>
        <header>
            <h3 class="title">Manage Applicants</h3>
            <p class="my-3">You currently have <span class="total-applicants"><?php echo count($row_application) ?></span> applicant/s.</p>
        </header>
    ...
    <!-- TABLE -->
        ...
        <!-- HEADER -->
            <thead role="rowgroup" class="headers">
                <tr role="row" class="headers">
                    ...
                    <th role="columnheader" class="py-3 pe-5">First Name</th>
                    <th role="columnheader" class="py-3 pe-5">Last Name</th>
        ...
        <!-- DATA -->
            <tbody role="rowgroup">
            <?php
                    for($i = 0; $i < count($row_application); $i++){
                        $propertyId = $row_application[$i]['property_id'];
                        $renterId = $row_application[$i]['renter_id'];
                ...
                <tr role="row" class="border-bottom">
                    <td role="cell" class="py-3 pe-2 manage-applicants-header"><?php echo $getProperty['propertyTitle'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-applicants-header"><?php echo $getrenterinfo['rFname'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-applicants-header"><?php echo $getrenterinfo['rLname'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-applicants-header"><?php echo $getrenterinfo['rNum'] ?></td>
                    <td role="cell" class="py-3 pe-2 manage-applicants-header"><?php echo $getrenterinfo['rEmail'] ?></td>
                    <td role="cell" class="py-3 manage-applicants-header">
                        <div class="d-flex flex-column gap-2">
                            <div class="">
                                <a href="approveApplication.php?property=<?php echo $propertyId ?>&id=<?php echo $getrenterinfo['rId']?>" class="btn btns-manage-applicants btn-mApprove d-flex align-items-center justify-content-center px-3 py-2" role="button">
                                    <i class="bi bi-search-heart-fill pe-1"></i>
                                    <span>View</span>
                                </a>
                            </div>
                            <div class="">
                                <a href="../messages.php?renterId=<?php echo $getrenterinfo['rId'] ?>" class="btn btns-manage-applicants btn-message d-flex align-items-center justify-content-center px-3 py-2" role="button">
                                    <i class="bi bi-chat-dots-fill actionIcon pe-1"></i>
                                    <span>Message</span>
                                </a>
                            </div>
                        </div>
    ...
    ?>
          <!-- THERE'S REJECTED APPLICANTS -->
          <div class="mt-3">
            <?php
            if(count($row_applicationrejected) > 0){
        ?>
            ...
            <table role="table" class="table-manage-applicants">
                ...
                <!-- DATA -->
                    <tbody role="rowgroup">
                        <?php
                        for($i = 0; $i < count($row_applicationrejected); $i++){
                            $propertyrejectedid = $row_applicationrejected[$i]['property_id'];
                            $renterrejectedId = $row_applicationrejected[$i]['renter_id'];
                            $selectrejectedProperty = "SELECT * FROM landing_properties WHERE propertyID='$propertyrejectedid'";
                            $executerejectedProperty = mysqli_query($con, $selectrejectedProperty);
                            $getPropertyrejected = mysqli_fetch_assoc($executerejectedProperty);
                
                            $selectrejectedrenter= "SELECT * FROM user_renter WHERE rId='$renterrejectedId'";
                            $executerejectedrenter = mysqli_query($con, $selectrejectedrenter);
                            $getrejectedrenterinfo = mysqli_fetch_assoc($executerejectedrenter);
                            ?>
                        <tr role="row" class="border-bottom">
                            <td role="cell" class="py-3 pe-2"><?php echo $getPropertyrejected['propertyTitle'] ?></td>
                                <td role="cell" class="py-3 pe-2"><?php echo $getrejectedrenterinfo['rFname'] ?></td>
                            <td role="cell" class="py-3 pe-2"><?php echo $getrejectedrenterinfo['rLname'] ?></td>
                            <td role="cell" class="py-3 pe-2"><?php echo $getrejectedrenterinfo['rNum'] ?></td>
                            <td role="cell" class="py-3 pe-2"><?php echo $getrejectedrenterinfo['rEmail'] ?></td>
                            <td role="cell" class="py-3">
                                <div class="d-flex flex-column gap-2">
                                    <div class="">
                                    </div>
                                    <div class="">
                                        <a href="../messages.php?renterId=<?php echo $getrejectedrenterinfo['rId'] ?>" class="btn btns-manage-applicants btn-message mReject btn-mMessage d-flex align-items-center justify-content-center" role="button">
                                            <i class="bi bi-chat-dots-fill actionIcon pe-1"></i>
                                            <span>Message</span>
                                        </a>
                                    </div>
    ...
    <div class="d-none">
        <input type="text" value="<?php echo count($row_application) ?>" id="currentAppCount">
    </div>
    ...
    <script>
        ...
        setInterval(function(){
            ...
            setInterval(function(){
                $.ajax({
                    url:"../Functions/Landlord/refreshApplicants.php",
                    method:"POST",
                    data:{
                        userid:$("#txtUserId").val(),
                        currentCount:$("#currentAppCount").val()
                    },
                    dataType:"text",
                    success:function(data)
                    {
